...
import { ArrowUpRight, CheckCircle2, Clock3, GitPullRequest, LocateFixed, ScanEye, Trash2, X } from 'lucide-react'
    ...
    >
      ...
      <div className="notification-list">
        {notices.length ? notices.map((notice) => (
          <article className={`notification-item is-${notice.kind} ${notice.read ? 'is-read' : 'is-unread'}`} key={notice.id}>
            <button
              type="button"
              className="notification-locate-button"
              onClick={() => onSelect(notice)}
              aria-label={`Mostrar PR ${notice.pullRequestId} en la cola${notice.read ? '' : ', sin leer'}`}
            />
            <span className="notification-icon" aria-hidden="true">
              {notice.kind === 'new-pr' ? <GitPullRequest size={17} /> : <ScanEye size={17} />}
            </span>
            <div className="notification-content">
              ...
              <div className="notification-meta">
                <span className="notification-kind">{notice.kind === 'new-pr' ? 'Nuevo PR' : 'Revisar'}</span>
                ...
                <span className="notification-read-state">{notice.read ? 'Leída' : 'Sin leer'}</span>
              </div>
              <span className="notification-locate-label"><LocateFixed size={13} aria-hidden="true" /> Mostrar en la cola</span>
            </div>
