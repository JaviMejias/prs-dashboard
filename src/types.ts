...
export type PullRequestActivity = { pull_request?: { id?: number }; comment?: { id?: number; parent?: { id?: number }; created_on?: string; updated_on?: string; deleted?: boolean; user?: ActivityUser }; approval?: { date?: string; user?: ActivityUser }; update?: { date?: string; author?: ActivityUser; state?: string; source?: { commit?: { hash?: string } } } }
...
export type ReviewDecisionReason = 'new_pr' | 'no_review_recorded' | 'reviewer_not_identified' | 'new_commit_after_review' | 'same_commit_after_review' | 'developer_activity_after_review' | 'reviewer_activity_after_review' | 'updated_after_review' | 'waiting_for_developer' | 'merged' | 'declined' | 'activity_incomplete'
export type ReviewDecisionConfidence = 'confirmed' | 'inferred' | 'unknown'
export type DeveloperActivityKind = 'commit' | 'comment' | 'commit_and_comment'
export type ReviewDecision = { status: PrStatus; reason: ReviewDecisionReason; confidence: ReviewDecisionConfidence; reviewDate?: string; reviewedCommit?: string; latestCommit?: string; latestDeveloperActivity?: DeveloperActivityKind }
...
export type ReviewContextComment = { id?: number; parentId?: number; author?: string; date?: string; content: string; path?: string; line?: number; url?: string }
