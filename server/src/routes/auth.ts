...
import { setSession, clearSession, authenticatedUserId } from '../auth.js'
import { query } from '../db/index.js'
import { log } from '../logger.js'
import { randomUUID } from 'node:crypto'

const loginSchema = z.object({ email: z.string().email(), token: z.string().min(1) })
...
export async function authRoutes(app: FastifyInstance) {
  app.post('/api/auth/login', async (request, reply) => {
    ...
    return { user: { id: userId, uuid, email: input.data.email, displayName: user.display_name } }
  })
  app.get('/api/auth/session', async (request, reply) => {
    const userId = authenticatedUserId(request)
    if (!userId) return reply.code(401).send({ error: 'No autenticado.' })
    const result = await query<{ id: string; bitbucket_uuid: string; email: string; display_name: string }>('SELECT id, bitbucket_uuid, email, display_name FROM users WHERE id = $1', [userId])
    if (!result.rows[0]) return reply.code(401).send({ error: 'Sesión inválida.' })
    const user = result.rows[0]
    return { user: { id: user.id, uuid: user.bitbucket_uuid, email: user.email, displayName: user.display_name } }
  })
